...
if ($FUNCTION_EXISTS) {
    ...
    try {
        $null = (aws lambda update-function-configuration --function-name $FUNCTION_NAME --runtime nodejs20.x --handler index.handler --timeout 15 --memory-size 128 --output text 2>$null)
    } catch {}
} else {
    Write-Host "Creating new function..."
    $null = (aws lambda create-function --function-name $FUNCTION_NAME --runtime nodejs20.x --role $ROLE_ARN --handler index.handler --zip-file fileb://function.zip --timeout 15 --memory-size 128 --output text)
